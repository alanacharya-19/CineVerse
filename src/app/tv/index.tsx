...
export default function TVShowsScreen() {
  ...
  const load = useCallback(async (p: number) => {
    setError(false);
    try {
      const data = await fetchTrendingTV(p);
      if (p === 1) setShows(data);
  ...
  const renderShow = ({ item: show }: { item: TVShow }) => (
    <TouchableOpacity
      activeOpacity={0.8}
    ...
    >
      <Image source={{ uri: show.poster_path }} className="w-full h-full" resizeMode="cover" />
      <View className="absolute bottom-0 left-0 right-0 p-3 bg-black/60">
        <Text className="text-white text-sm font-bold" numberOfLines={1}>{show.title}</Text>
    ...
    return (
      <View className="flex-1" style={{ backgroundColor: colors.background }}>
        ...
        <View className="flex-row flex-wrap px-4 gap-3">
          {[1, 2, 3, 4].map((i) => (
            <Skeleton key={i} className="rounded-2xl" style={{ width: CARD_W, height: CARD_H }} />
          ))}
        </View>
      </View>
    );
      ...
      <View className="flex-1" style={{ backgroundColor: colors.background }}>
        <SafeAreaView className="mb-3">
          <View className="flex-row items-center gap-3 mx-4">
            <TouchableOpacity onPress={() => router.back()} className="w-9 h-9 rounded-full items-center justify-center" style={{ backgroundColor: colors.iconBg }}>
        ...
        <View className="flex-1 items-center justify-center">
          ...
          <TouchableOpacity onPress={() => { setLoading(true); load(1); }} className="rounded-xl px-5 py-2" style={{ backgroundColor: colors.accent + "20" }}>

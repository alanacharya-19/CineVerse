...
export default function UpcomingScreen() {
  ...
  const load = useCallback(async (p: number) => {
    setError(false);
    try {
      const data = await fetchUpcoming(p);
      const sorted = [...data].sort(
        (a, b) => new Date(a.releaseDate).getTime() - new Date(b.releaseDate).getTime()
      );
      if (p === 1) setMovies(sorted);
      else setMovies((prev) => [...prev, ...sorted]);
      setPage(p);
    } catch {
      setError(true);
    } finally {
    ...
    >
      ...
      <View className="absolute bottom-0 left-0 right-0 p-3 bg-black/60">
        ...
        <View className="flex-row items-center gap-1 mt-0.5">
          <Ionicons name="calendar" size={10} color={colors.textMuted} />
          <Text className="text-white text-[10px]">{movie.releaseDate}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
